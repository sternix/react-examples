...
function MineSweeperView({
  ...
  const remainMines = () => {
    return (
      mines - ceils.filter(ceil => ceil.state === 'flag' || ceil.state === 'misflagged').length
    );
  }
  ...
  }, [openBehavior, openingCeil, openingCeils]);

  const onMouseDownCeils = (e, index) => {
